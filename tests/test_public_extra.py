...
class TestMarcosObtemIntersecoesAdjacentes:
    def test_1(self):  # Canto cima esquerdo
        ...
        answer = ["B1", "A2"]
        assert obtem_intersecoes_adjacentes(i1, i2) == tuple(
            str_para_intersecao(x) for x in answer
        )
...
class TestMarcosEhJogadaLegal:
    ...
    def test_3(self):  # Regra do KO
        ...
        assert not eh_jogada_legal(
            g, cria_intersecao("A", 1), cria_pedra_branca(), copy
        )
